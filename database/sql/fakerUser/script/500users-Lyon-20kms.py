...
# Output file name
output_file="../resultSQL/fake_profiles-500users-Lyon-20kms.sql"

# OpenCageData API key
opencage_api_key = os.getenv('KEY_OPENCAGE2')  # Replace with your actual API key OPENCAGE

# Base location coordinates (e.g., Lyon)
base_lat = 45.7640  # Latitude of Lyon
base_lon = 4.8357   # Longitude of Lyon
...
start_user_id = 2100
start_profile_id = 2100
start_photo_id = 2100
start_location_id = 2100
...
genders = [
    ...
    {'id': 7, 'name': 'Non-binary', 'photo_gender': 'men'},  # Alternatif possible
    {'id': 8, 'name': 'Gender fluid', 'photo_gender': 'women'},  # Alternatif possible
    {'id': 9, 'name': 'Agender', 'photo_gender': 'men'},  # Alternatif possible
    {'id': 10, 'name': 'Genderqueer', 'photo_gender': 'women'},  # Alternatif possible
    {'id': 11, 'name': 'Gender non-conforming', 'photo_gender': 'men'},  # Alternatif possible
    {'id': 12, 'name': 'Transgender (MTF)', 'photo_gender': 'women'},
    {'id': 13, 'name': 'Transgender (FTM)', 'photo_gender': 'men'},
    {'id': 14, 'name': 'Questioning', 'photo_gender': 'women'}  # Alternatif possible
]
...
# Loop to generate 1000 users and profiles
for i in range(numberUserGenerated):
    current_user_id = start_user_id + i
    current_profile_id = start_profile_id + i
    current_photo_id = start_photo_id + i
    current_location_id = start_location_id + i
    ...
    main_photo_url = generate_photo_url(gender['photo_gender'], i % 100)
    ...
    print(str(i) + '  generate ' + username + ' at location ' + city_name)
...
with open(output_file, "w", encoding="utf-8") as file:
    ...
    file.write("-- SQL commands to create 1000 profiles\n")
