...
class LikesDAL {
    async getLikesByUserId(userId: number): Promise<Like[]> {
        try {
            ...
            throw {status: error.status, message: 'Impossible de récupérer les likes pour cet utilisateur'};
        }
    }
    ...
    async getLikesFromUser(userId: number): Promise<number[]> {
        try {
            ...
            throw {status: error.status, message: 'Impossible de récupérer les IDs des likes'};
        }
    }
